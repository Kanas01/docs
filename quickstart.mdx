...
<AccordionGroup>
  <Accordion icon="file-pen" title="填入API配置">
    ...
    ![title](/images/env_local.png)
    使用记事本或其他编辑器打开：
    ![title](/images/env_local_2.png)
    打开后，在`COMFLY API KEY=`处填入您sk-开头的API Key，在`COMFLY BASE URL=`处填入您选择的API调用网站：
    ![title](/images/fill_in_api.png)
...
<Accordion icon="rocket" title="开始使用Nody">
  ...
  ![title](/images/nody_exe.png)
  打开会跳出命令行窗口：
  ![title](/images/bootstrap_1.png)
  片刻等待后启动完成，浏览器页面会自动打开：
  ![title](/images/bootstrap_success.png)
  了解界面后即可轻松使用！
